// Map file parser for Pokémon NDS games (Gen IV: DPPt/HGSS)
// Adapted from LiTRE project
...
namespace Clockwork.Core.Formats.NDS.MapFile
{
   ...
   ************* BDHC section
   ...
   -------------------------------------------------------------------------------------- */
    ...
    public class MapFile
    {
        ...
        public MapFile(string path, GameFamily gFamily = GameFamily.DPPt, bool discardMoveperms = false, bool showMessages = true)
            : this(new FileStream(path, FileMode.Open), gFamily, discardMoveperms, showMessages) { }

        public MapFile(Stream data, GameFamily gFamily = GameFamily.DPPt, bool discardMoveperms = false, bool showMessages = true)
        {
            using (BinaryReader reader = new BinaryReader(data))
            {
                ...
                if (gFamily == GameFamily.HGSS)
                {
                    ...
                    else
                    {
                        ...
                        if (showMessages)
                        {
                            throw new InvalidDataException("BGS header corrupted");
                        }
                    }
                }
        ...
        public bool LoadMapModel(byte[] newData, bool showMessages = true)
        {
            ...
            {
                if (modelReader.ReadUInt32() != NSBMD.NSBMD.NDS_TYPE_BMD0)
                {
                    if (showMessages)
                    {
                        throw new InvalidDataException("Invalid NSBMD file");
                    }
                    return false;
                }

                modelReader.BaseStream.Position = 0xE;
                if (modelReader.ReadInt16() > 1) // NSBMD contains embedded textures
                {
        ...
        public byte[] ToByteArray(GameFamily gFamily = GameFamily.DPPt)
        {
            MemoryStream newData = new MemoryStream();
            using (BinaryWriter writer = new BinaryWriter(newData))
            {
    ...
    public class Building
    {
        ...
        public void LoadModelData(string path)
        {
            ...
            {
                throw new FileNotFoundException($"Building model {modelID} not found at {path}");
            }
            ...
            {
                throw new FileNotFoundException($"Building model {modelID} not found");
            }
        }
    }
}
